...
```bash
PYTHONPATH=src /home/nxank4/.cache/cachepawl/vllm-cachepawl-venv/bin/python benchmarks/scripts/capture_vllm_baseline.py --output-dir /tmp/cachepawl-proof-current-len2048-seq2 --model "Zyphra/Zamba2-2.7B-instruct" --fallback-model "tiiuae/Falcon-H1-1.5B-Instruct" --timestamp "2026-06-05T01:01:00Z" --max-model-len 2048 --gpu-memory-utilization 0.7 --max-num-seqs 2 --gpu-total-bytes 12884901888 --gpu-name "NVIDIA GeForce RTX 3060" --runtime-timeout-seconds 1200 --generation-smoke --generation-timeout-seconds 60 --generation-prompt "Cachepawl long-ish bounded prompt for admission replay setup. Repeat context once for a longer prefill path." --max-new-tokens 1
```

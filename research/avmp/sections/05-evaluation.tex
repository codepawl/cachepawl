...
\caption{Cross-workload OOM and fragmentation baseline comparison ($\downarrow$ lower is better).}
...
\caption{Per-workload goodput comparison between AVMP and best static baseline; goodput columns $\uparrow$ higher is better, ratio shown without direction.}
...
\caption{Stage 1 migration batch size sweep: per-workload OOM distribution across 9 batch sizes ($\downarrow$ lower is better).}

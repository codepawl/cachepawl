...
\caption{Cross-workload OOM and fragmentation baseline comparison ($\downarrow$ lower is better). \textbf{Bold} = best per column; \underline{underline} = second best. The $\Delta\%$ column reports the change in total OOMs versus \texttt{padded\_unified}: negative numbers mark improvement.}
...
Table~\ref{tab:per-workload-winner} summarizes per-workload goodput. AVMP records 434.24 req/s on \texttt{uniform\_short} versus 32.65 req/s for \texttt{fixed\_dual\_mr05}, a 13.299$\times$ ratio. On \texttt{mixed\_long}, 65.07 req/s versus 27.25 req/s gives 2.388$\times$. On \texttt{agentic\_burst}, 46.91 req/s versus 25.69 req/s gives 1.826$\times$.

\begin{table*}[t]
\centering
\input{tables/generated/table_per_workload_winner.tex}
\caption{Per-workload goodput comparison between AVMP and best static baseline; goodput columns $\uparrow$ higher is better, ratio shown without direction. \textbf{Bold} = winner per row.}
...
\caption{OOM occurrence variance across different migration batch sizes ($\downarrow$ lower is better).}
...
\caption{Stage 1 migration batch size sweep: per-workload OOM distribution across 9 batch sizes ($\downarrow$ lower is better). \textbf{Bold} = best per column; \underline{underline} = second best.}
...
\caption{Threshold sensitivity analysis showing marginal effect on OOM events ($\downarrow$ lower is better).}
...
\caption{Stage 2 threshold sweep at b128: all four variants yield byte-identical OOM, rebalance count, and migration totals.}
...
\caption{Peak reserved VRAM trade-off for dynamic allocation; lower VRAM is better in absolute terms, but AVMP intentionally trades 2$\times$ VRAM for capacity migration headroom.}

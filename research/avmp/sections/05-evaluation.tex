...
\caption{Wall-clock phase decomposition per (variant, workload) cell ($\downarrow$ lower OOM-retry is better). Each bar normalizes service / OOM retry / migration / idle to a fraction of cell wall time, taking the median across 12 (model, pool, seed) cells. Service includes migration time as a sub-component; for stacked display we plot ``service excluding migration''. On \texttt{mixed\_long} and \texttt{agentic\_burst} \texttt{fixed\_dual\_mr05} spends 26\% / 10\% of wall time in OOM retry; \texttt{avmp\_dynamic\_b128} cuts that to 8.5\% / 2.1\%. On \texttt{uniform\_short} both variants OOM-retry at $\approx 0$\%, so the 13.30$\times$ goodput gap arises from a different source: \texttt{fixed\_dual\_mr05}'s per-call allocator service time is 32.2 s for the 512-request stream versus 2.1 s for AVMP at the same pool budget.}
...
\begin{table*}[t]
\centering
\caption{ShareGPT trace replay: per-variant aggregates and paired bootstrap CIs (B=10000, RNG seed 20260520) over the 12-cell (model, pool, seed) grid. Goodput ratio is the ratio of means with the same paired-bootstrap resampling; \textbf{*} marks CIs that exclude the null. \texttt{fixed\_dual\_mr05} is the baseline. All AVMP variants record the same OOM count as \texttt{fixed\_dual\_mr05}; the goodput advantage comes from faster per-call service rather than from OOM avoidance.}
...
\caption{AVMP goodput ratio vs \texttt{fixed\_dual\_mr05} across the three synthetic workloads and the ShareGPT replay ($\uparrow$ higher is better; dashed line = unity). Error bars are paired bootstrap 95\% CIs. ShareGPT lands at 2.36$\times$, between \texttt{agentic\_burst}'s 1.83$\times$ and \texttt{mixed\_long}'s 2.39$\times$, and well below \texttt{uniform\_short}'s 13.30$\times$. The synthetic short-prompt extreme inflates the headline number; ShareGPT gives a more realistic central-tendency estimate of the per-call-service-speedup mechanism.}

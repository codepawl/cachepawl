...
| Tier 1 PR B throughput | Schema 1.2.0 adds effective_batch_size_{mean,p50,p95,p99}, goodput_requests_per_second, completion_ratio (strict: no OOM during request lifetime AND clean free), and time_to_first_oom_seconds. 180-cell GPU sweep on the 5-variant `throughput_v2` preset (baselines + `avmp_dynamic_b128`), 16:14 wall. Sweep at [`benchmarks/results/avmp-v2-throughput/full/`](../../benchmarks/results/avmp-v2-throughput/full/). Pre-registered stop rule verdict: **PASS_GOODPUT** (avmp_dynamic_b128 goodput >= 1.10x fixed_dual_mr05 on all 3 workloads: uniform_short 13.30x, mixed_long 2.39x, agentic_burst 1.83x). Batch-size hypothesis PARTIALLY FALSIFIED: effective_batch_size_p50 is workload-dominated (129/132/284 across workloads), identical across all five variants. 3-level lex ranking confirms avmp_dynamic_b128 at rank 1 on cross-workload total_oom (510.0 vs fd05 552.0, -7.6%). The paper's throughput claim rests on goodput, not on sustained concurrency. See [`throughput_analysis.md`](../../benchmarks/results/avmp-v2-throughput/full/throughput_analysis.md) |

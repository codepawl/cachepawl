#!/usr/bin/env python
"""Capture bounded read-only Mamba/attention contract metadata from vanilla vLLM."""
...
DEFAULT_OUTPUT_DIR = Path("research/avmp/v2/results/vllm-mamba-attention-contract-observation")
...
OBSERVATION_PREFIX = "CACHEPAWL_MAMBA_ATTENTION_CONTRACT_OBSERVATION="
...
def main(argv: list[str] | None = None) -> int:
    ...
    capture_mamba_attention_contract_observation(
...
def capture_mamba_attention_contract_observation(
    *,
    output_dir: Path,
    timestamp: str,
    model: str,
    ...
    payload = create_mamba_attention_contract_artifact_payload(
        timestamp=timestamp,
        model=model,
        prompt=prompt,
        vllm_version=vllm_version,
    ...
    write_mamba_attention_contract_artifact(output_dir, payload)
    return payload


def create_mamba_attention_contract_artifact_payload(
    *,
    timestamp: str,
    model: str,
    prompt: str,
    ...
    report: JsonObject = {
        "status": "mamba_attention_contract_observation_available",
    ...
    manifest: JsonObject = {
        "artifact": "vllm-mamba-attention-contract-observation",
        "timestamp": timestamp,
        ...
        "outputs": {
            ...
            "mamba_attention_contract_report": "mamba_attention_contract_report.json",
            "summary": "summary.md",
    ...
    return {
        "manifest": manifest,
        "mamba_attention_contract_report": report,
        "raw_safe_metadata": _as_json_object(child_payload["raw_safe_metadata"]),
        "field_level_blockers": blockers,
    }


def write_mamba_attention_contract_artifact(output_dir: Path, payload: JsonObject) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "manifest.json").write_text(_json_dumps(_as_json_object(payload["manifest"])))
    (output_dir / "mamba_attention_contract_report.json").write_text(
        _json_dumps(_as_json_object(payload["mamba_attention_contract_report"]))
    )
...
def _run_child(command: list[str], *, timeout_seconds: int) -> JsonObject:
    try:
        ...
        return {
            "status": "timeout",
            "reason": "Mamba/attention contract observation timed out",
    ...
    status = (
        "completed"
        if completed.returncode == 0 and payload_status == "mamba_attention_contract_observation"
        else "failed"
    )
    return {
        "status": status,
        "reason": (
            "Mamba/attention contract observation completed"
            if status == "completed"
    ...
    *,
    ...
    code = "\n".join(
        [
            ...
            "    ignore_eos=True,",
            ")",
            "payload = observe_vllm_mamba_attention_contract(",
            "    llm,",
            f"    prompt={prompt!r},",
...
def _classification(blockers: list[object]) -> str:
    return (
        ...
        if blockers
        else "mamba_attention_contract_observation_complete"
    )
...
def _write_blocker(output_dir: Path, blocker: JsonObject) -> None:
    ...
    for stale in (
        "mamba_attention_contract_report.json",
...
def _summary(payload: JsonObject) -> str:
    ...
    report = _as_json_object(payload["mamba_attention_contract_report"])
    ...
    return f"""# vLLM Mamba Attention Contract Observation Summary
...
def _readme(payload: JsonObject) -> str:
    manifest = _as_json_object(payload["manifest"])
    return f"""# vLLM Mamba Attention Contract Observation

Status: `{manifest["classification"]}`

This artifact captures bounded read-only Mamba state-index, attention group, and
block-table metadata from vanilla `vllm=={manifest["vllm_version"]}` for
`{manifest["model"]}`.
...
No vLLM source edits, monkeypatching, allocator replacement, scheduler mutation,
worker layout mutation, returned Cachepawl plans, controlled substitution,
Triton kernels, copy kernels, LSDR, serving changes, or quality evaluation were
performed.
"""
...
def _payload_failure_reason(payload: JsonObject | None) -> str:
    if payload is None:
        return "Mamba/attention contract observation failed before emitting a payload"
    reason = payload.get("unsupported_reason")
    if reason is not None:
        return str(reason)
    status = payload.get("status")
    return f"Mamba/attention contract observation returned unsupported status {status!r}"
...
def _parse_payload(stdout: str) -> JsonObject | None:
    for line in stdout.splitlines():
        if line.startswith(OBSERVATION_PREFIX):
            return _as_json_object(json.loads(line.removeprefix(OBSERVATION_PREFIX)))
    return None

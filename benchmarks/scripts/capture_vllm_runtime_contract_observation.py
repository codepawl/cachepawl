#!/usr/bin/env python
"""Capture a read-only runtime-contract observation from vanilla vLLM."""
...
DEFAULT_OUTPUT_DIR = Path("research/avmp/v2/results/vllm-runtime-contract-observation")
...
OBSERVATION_PREFIX = "CACHEPAWL_RUNTIME_CONTRACT_OBSERVATION="
...
def main(argv: list[str] | None = None) -> int:
    ...
    capture_runtime_contract_observation(
...
def capture_runtime_contract_observation(
    *,
    output_dir: Path,
    timestamp: str,
    model: str,
    ...
    command = _runtime_contract_command(
        model=model,
        max_model_len=max_model_len,
        gpu_memory_utilization=gpu_memory_utilization,
        max_num_seqs=max_num_seqs,
    ...
    result = _run_runtime_contract_child(command, timeout_seconds=timeout_seconds)
    ...
    payload = create_runtime_contract_artifact_payload(
        timestamp=timestamp,
        model=model,
        vllm_version=vllm_version,
        max_model_len=max_model_len,
    ...
    write_runtime_contract_artifact(output_dir, payload)
    return payload


def create_runtime_contract_artifact_payload(
    *,
    timestamp: str,
    model: str,
    ...
    report: JsonObject = {
        "status": "runtime_contract_observation_available",
        "classification": _classification(blockers),
        "model": model,
        "runtime_path": child_payload["runtime_path"],
        "scheduler_manager": _as_json_object(child_payload["scheduler_manager"]),
        "block_usage": _as_json_object(child_payload["block_usage"]),
        "worker_tensors": _as_json_array(child_payload["worker_tensors"]),
        "mamba_attention": _as_json_object(child_payload["mamba_attention"]),
        "fields": fields,
    ...
    manifest: JsonObject = {
        "artifact": "vllm-runtime-contract-observation",
        "timestamp": timestamp,
        ...
        "outputs": {
            ...
            "runtime_contract_report": "runtime_contract_report.json",
            "summary": "summary.md",
...
def write_runtime_contract_artifact(output_dir: Path, payload: JsonObject) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    (output_dir / "runtime_contract_report.json").write_text(
        _json_dumps(_as_json_object(payload["runtime_contract_report"]))
    )
...
def _run_runtime_contract_child(command: list[str], *, timeout_seconds: int) -> JsonObject:
    ...
    try:
        ...
        return {
            "status": "timeout",
            "reason": "runtime contract observation timed out",
    ...
    status = (
        "completed"
        if completed.returncode == 0 and payload_status == "runtime_contract_observation"
        else "failed"
    )
    return {
        "status": status,
        "reason": (
            "runtime contract observation completed"
            if status == "completed"
...
def _runtime_contract_command(
    *,
    ...
    code = "\n".join(
        [
            "import json",
            "from cachepawl.integrations.vllm import observe_vllm_runtime_contracts",
            ...
            ")",
            "payload = observe_vllm_runtime_contracts(llm).to_dict()",
...
def _payload_failure_reason(payload: JsonObject | None) -> str:
    if payload is None:
        return "runtime contract observation failed before emitting a payload"
    reason = payload.get("unsupported_reason")
    if reason is not None:
        return str(reason)
    return f"runtime contract observation returned unsupported status {payload.get('status')!r}"


def _classification(blockers: list[object]) -> str:
    return (
        "runtime_contract_observation_with_field_blockers"
        if blockers
        else "runtime_contract_observation_complete"
    )
...
def _summary(payload: JsonObject) -> str:
    ...
    report = _as_json_object(payload["runtime_contract_report"])
    ...
    return f"""# vLLM Runtime Contract Observation Summary
...
def _readme(payload: JsonObject) -> str:
    manifest = _as_json_object(payload["manifest"])
    return f"""# vLLM Runtime Contract Observation
